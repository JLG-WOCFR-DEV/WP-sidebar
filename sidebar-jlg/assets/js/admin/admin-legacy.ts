...
const loadSearchModule = () => import(/* webpackChunkName: "sidebar-admin-search" */ './search');
const loadProfilesModule = () => import(/* webpackChunkName: "sidebar-admin-profiles" */ './profiles');
const loadHeaderModule = () => import(/* webpackChunkName: "sidebar-admin-header" */ './header');
...
jQuery(document).ready(function($) {
    ...
    let profilesApi = null;
    let profilesModulePromise = null;
    ...
    previewModule.loadPreview = function(...args) {
        return originalLoadPreview(...args).then((response) => {
            if (profilesApi && typeof profilesApi.updateActiveProfileStatusMessage === 'function') {
                profilesApi.updateActiveProfileStatusMessage();
            } else if (profilesModulePromise) {
                profilesModulePromise.then((api) => {
                    if (api && typeof api.updateActiveProfileStatusMessage === 'function') {
                        api.updateActiveProfileStatusMessage();
                    }
                });
            }
            return response;
    ...
    const searchModulePromise = loadSearchModule().then(({ initSearchSection }) => initSearchSection($, getI18nString));
    profilesModulePromise = loadProfilesModule().then(({ initProfilesSection }) => {
        profilesApi = initProfilesSection($, previewModule, renderNotice, getI18nString);
        return profilesApi;
    });
    loadHeaderModule().then(({ initHeaderSection }) => initHeaderSection($));
